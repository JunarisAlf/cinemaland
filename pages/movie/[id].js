...
export async function getStaticProps({params}){
    const resMovie = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=${process.env.api_key}`)

    // const resCredits = await fetch(`https://api.themoviedb.org/3/movie/580489/credits?api_key=92af204d0915b0e4dbff993d8d49ebae`)
    const resCredits = await fetch(`https://api.themoviedb.org/3/movie/${params.id}/credits?api_key=${process.env.api_key}`)
